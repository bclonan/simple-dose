...
test('documentation matches native discovery and registration survives both new routes', async ({ page }) => {
  ...
  await expect(page.getByTestId('registered-tool-count')).toHaveText('20')
  const names = await runtimeNames(page)
  expect(await documentedNames(page)).toEqual(names)
  expect(names).toHaveLength(20)
  expect(names).toContain('prepare_demo_checkout')
  await expect(page.getByRole('heading', { name: '6 chained workflows', exact: true })).toBeVisible()
  await expect(page.locator('#chained-workflows > .workflow-card')).toHaveCount(6)
...
test('both documentation routes work without WebMCP and fit mobile through desktop', async ({ page }) => {
  ...
  for (const route of ['/webmcp', '/hackathon']) {
    ...
    if (route === '/webmcp') {
      ...
      expect(await documentedNames(page)).toHaveLength(20)
