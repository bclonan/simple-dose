...
import { drugFactStatus } from '../domain/drug-fact-status'
  ...
  function getFact(drugIds: readonly string[], factType: DrugFactType) {
    ...
    return {
      ...
      drugs: drugIds.map(drugId => {
        ...
        const outcome = drugFactStatus(record, factType, loading, demoMode)
        const failedLabel = definition.load === 'clinical' ? 'FDA label failed to load' : 'Public source failed to load'
        const missingLabel = definition.load === 'clinical'
          ? outcome.availability === 'field-absent' ? 'FDA section unavailable' : 'FDA label unavailable'
          : 'Requested public fact unavailable'
        const factStatusLabel = outcome.availability === 'provider-failed' ? failedLabel
          : outcome.availability === 'partial' ? 'Available data; source refresh failed'
            : ['field-absent', 'source-unavailable', 'not-loaded'].includes(outcome.availability) ? missingLabel : statusLabels[record?.status ?? 'unavailable']
        return {
          drugId, label: record?.drug?.identity.genericName ?? catalog.medicationById(drugId)?.genericName ?? drugId,
          loading, status: record?.status ?? 'unavailable', availability: outcome.availability,
          statusLabel: loading ? 'Loading requested facts...' : record?.status === 'demo' && !demoMode ? 'Demo fallback, public facts unavailable' : factStatusLabel,
          failureMessage: outcome.availability === 'partial' ? outcome.message : undefined,
          message: record?.message,
          emptyMessage: record?.status === 'demo' ? demoMode ? 'Demo mode does not provide public clinical facts. Switch to hybrid or live data to request this information.' : 'Public facts are unavailable. Fictional demo fulfillment data does not supply clinical information. Retry public data when connected.' : loading ? 'Loading this fact from the shared medication record.' : outcome.message || definition.emptyMessage,
